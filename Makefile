...
PROJECT     ?= kubetop
REPO        ?= github.com/ricoberger/kubetop
PWD         ?= $(shell pwd)
...
release: clean build-darwin-amd64 build-linux-amd64 build-windows-amd64
	...
	cp ${PWD}/bin/kubetop-darwin-amd64 ${PWD}/releases/${VERSION}
	cp ${PWD}/bin/kubetop-linux-amd64 ${PWD}/releases/${VERSION}
	cp ${PWD}/bin/kubetop-windows-amd64.exe ${PWD}/releases/${VERSION}
